...
onde o primeiro passo segue da definição de composição de funções; a desigualdade seguinte decorre da convexidade de $\tau$ e do fato de que $\phi$ é monotônica crescente; o terceiro passo segue de que $\phi$ é convexa; e a última igualdade segue, novamente, da definição de composição de funções.
		 ...
		 $$\lVert X_n \rVert_p \leq \lVert X_n \rVert_q\, .$$
			...
			Observe que $|X_n|\uparrow |X|$. Mas então, segue da aplicação do teorema da convergência monótona que:
	...
	\begin{proof}
		...
		Consideremos, então, o caso em que $\lVert Y \rvert_q > 0$. Nesse caso, $\mathbb{E}[|Y|^q]$ e, da nossa discussão sobre densidades da aula anterior, sabemos que o mapa $\tilde P: \Sigma \mapsto [0,1]$ definido por:
	...
	$	\lVert X \rVert_{\infty} = \inf \{C \geq 0: \mathbb{P}[{\{|X|>C\}}] = 0\} $
		...
		\begin{proof}
			...
			\begin{itemize}
				...
				onde usamos que $ \lim_{n \to \infty} \mathbb{P}[|X| > K]^{\frac{1}{n}} = 1$, pois  $\mathbb{P}[|X| > K] = 1$. Mas, como $K$ foi arbitrariamente escolhido, segue que, tomando $K \to \infty$, $\lVert X \rVert_\infty = \infty$.
