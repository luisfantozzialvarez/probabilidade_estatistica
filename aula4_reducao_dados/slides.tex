...
\begin{document}
	...
	\begin{frame}{Família dominada}
		\begin{itemize}
			\item No que segue, supomos que existe uma medida $\mu$ $\sigma$-finita tal que todo elemento $P_\theta \in \mathcal{P}$ admite densidade $p_\theta$ com respeito a $\mathcal{\mu}$ (nesse caso, dizemos que a família $\mathcal{P}$ é \emph{dominada}).
			\begin{halfwideitemize}
	...
	\begin{frame}{Condição técnica}
		\begin{itemize}
			...
			\begin{halfwideitemize}
				...
				\begin{itemize}
					\item Satisfeita para $(\Omega,\Sigma) = (\prod_{i=1}^n (\mathbb{R}^k), \otimes_{i=1}^n \mathcal{B}(\mathbb{R}^k))$.
				\end{itemize}
				\item Na maior parte dos casos práticos é satisfeita.
				\begin{itemize}
						\item Condições suficientes fora do escopo do curso.
				\end{itemize}
			\end{halfwideitemize}
	...
	\begin{frame}{Exemplo}
		...
		\begin{equation*}
			\begin{aligned}         
				...
				= \frac{\theta^{x+y} (1-\theta)^{2-x-y}\mathbf{1}_{\{x+y = 2t\}}}{\binom{2}{2t} \theta^{2t}(1-\theta)^{2-2t}}= \frac{1}{\binom{2}{2t} } \mathbf{1}_{\{x+y = 2t\}} \, ,
			\end{aligned}
		\end{equation*}
...
\end{frame}
	...
	\begin{frame}{Exemplo}
		...
		\begin{equation*}
			\begin{aligned}
				p_\lambda(x_1,x_2,\ldots x_n) = \prod_{i=1}^n (\lambda e^{-\lambda x_i} \mathbf{1}_{\{x_i>0\}}) =\\ =  \lambda^n \exp\left(- \lambda \sum_{i=1}^n x_i\right)\mathbf{1}_{\{\min_i x_i > 0 \}} = h_\lambda \left(\sum_{i=1}^n x_i\right) c(\boldsymbol{x}) \, ,
			\end{aligned}
		\end{equation*}
	...
	\begin{frame}{Exemplo}
		...
		$$p_\theta(x_1,\ldots, x_n) = \frac{1}{\theta^n} \mathbf{1}_{\{\max_i x_i < \theta\}} \, .  $$
	...
	\begin{frame}{Suficiência completa vs. suficiência minimal}
		...
		\end{theorem}
		\begin{halfwideitemize}
			\item Recíproca {\color{red}\textbf{não}} é verdadeira: existem estatísticas minimais suficientes de dimensão finita que não são completas (veja o Exemplo 6.25 \cite{lehmann2006theory}).
		\end{halfwideitemize}
	...
	\begin{frame}{Identificabilidade de famílias de distribuições}
		...
		\begin{halfwideitemize}
			...
			\end{halfwideitemize}
			\item No exemplo anterior, $\theta$ é identificado, visto que $f_1(P_\theta) := \mathbb{E}_\theta[X_1] = \mu$ e $f_2(P_\theta) := \mathbb{E}_\theta[X_1^2] = \sigma^2 + \mu^2$.
			\item Por outro lado, se a família paramétrica fosse $\{\mathcal{N}( \theta \lor 0, 1): \theta \in \mathbb{R}\}$, $\theta$ não é identificável.
		\end{halfwideitemize}
	\end{frame}
	\begin{frame}{Reparametrização natural}
		...
		$$p_{\boldsymbol{\eta}}(\boldsymbol{x}) = \exp(\boldsymbol{\eta}'T(\boldsymbol{x}) -B^*(\boldsymbol{\eta})) h(\boldsymbol{x}) \, , \quad \boldsymbol{\eta} \in \Xi \, ,$$
		...
		\begin{block}{Definição}
			Uma família exponencial $\{P_\theta: \theta \in \Theta\}$ é dita uma família de posto cheio se sua reparametrização natural é tal que $\boldsymbol{\eta}$ é identificável e $\Xi$ contém uma bola aberta de $\mathbb{R}^s$.
		\end{block}
		\begin{halfwideitemize}
			\item No exemplo de amostra aleatória de população normal, $\Xi = (-\infty,0) \times \mathbb{R}$, e, como $\theta$ é identificável, $\boldsymbol{\eta}$ também o é, logo a família tem posto cheio.
	...
	\begin{itemize}
		\item Considere um experimento estatístico $(\Omega,\Sigma, P)$, e um modelo de leis de probabilidade candidatas para $P$, $\mathcal{P} = \{P_\theta:\theta \in \Theta\}$, com $P=P_{\theta_0}$ para algum $\theta_0 \in \Theta$.
	...
	\end{frame}
		...
		\begin{theorem}
			Um estimador $\delta$ de variância finita e não viciado para  $\psi(\theta)$ em $\mathcal{P}$ é não viciado de variância uniformemente mínima em $\mathcal{P}$ se, e somente se, para qualquer estimador $U$ não viciado para zero em $\mathcal{P}$:
			
			$$\forall \theta \in \Theta\, , \mathbb{V}_\theta[U]<\infty \implies  \mathbb{E}_\theta[\delta(\boldsymbol{X}) U(\boldsymbol{X})] = 0\, .$$
		\end{theorem}
		\begin{corollary}
	...
	\end{frame}
		...
		\begin{itemize}
			...
			\begin{itemize}
				\item Encontrar $\delta$ resolvendo o ``sistema de equações'': $\mathbb{E}_\theta[\delta(\boldsymbol{X})]=\psi(\theta)$, $\mathbb{E}_\theta[\delta(\boldsymbol{X}) U(\boldsymbol{X})] = 0$ para todo $\theta$ e $U$ não viciado para zero (tais que $\mathbb{V}_\theta[U]<\infty$).
	...
	\begin{frame}{Limite inferior de Crámer-Rao: derivação}
		\begin{itemize}
			...
			\item Defina a variável aleatória $S_{\theta,\boldsymbol{e}} = \nabla_\tau \log p_{\theta}(\boldsymbol{X})\cdot \boldsymbol{e} =  \frac{\nabla_{\tau} p_{\theta}(\boldsymbol{X}) \cdot \boldsymbol{e}}{p_{\theta}(\boldsymbol{X})}$.
				\item Observe que, como $\int p_{\tau}(x)\mu(dx)=1$ para todo $\tau$, se pudermos diferenciar em $\tau$ por dentro da integral, segue que $\mathbb{E}_{\theta}[S_{\theta,\boldsymbol{e}}] = 0$ e que $\operatorname{cov}_{\theta}(\delta, S_{\theta,\boldsymbol{e}}) = \int\delta(x)  (\nabla_{\tau} p_{\theta}(x) \cdot \boldsymbol{e})  \mu(dx)$.
