...
\title{Redução de Dados}
\subtitle{Princípios de Redução de Dados}
%\logo{aa}
...
\begin{document}
	...
	\begin{frame}{Panorama desta aula}
		\begin{halfwideitemize}
			\item Nesta aula, nós discutiremos o conceito de suficiência e seus derivados.
			\begin{halfwideitemize}
				\item Conceitos capturam ideia crucial de {\color{blue}redução de dados}: como reduzir a informação nos dados a um conjunto ``pequeno'' de estatísticas, {\color{blue}sem perder informação relevante para o problema de interesse?} 
				\item Estes conceitos são úteis na construção de estimadores e testes de hipóteses com propriedades desejáveis.
			\end{halfwideitemize}
			\item Em seguida, nós veremos que, em {\color{blue}famílias exponenciais}, a redução de dados toma uma forma bastante conveniente.
			\begin{halfwideitemize}
				\item Veremos que diversas distribuições conhecidas pertencem à família exponencial.
				\item Veremos, sob quais condições estatísticas, com boas propriedades de redução de dados são facilmente recuperáveis em famílias exponenciais
			\end{halfwideitemize}
			\item Referências desta aula: \citet{Casella2001}, com tópicos de \citet{Schervish1995},  \citet{Lehmann2005} e \citet{lehmann2006theory}.
		\end{halfwideitemize}
	\end{frame}
	...
	\end{frame}
	\begin{frame}{Hipóteses técnicas}
		\begin{halfwideitemize}
			\item No que segue, supomos que existe uma medida $\mu$ tal que todo elemento de $P_\theta \in \mathcal{P}$ admite densidade $p_\theta$ com respeito a $\mathcal{\mu}$.
			\begin{halfwideitemize}
				\item Para famílias de distribuições discretas, $\mu$ é a medida de contagem e $p_\theta$ são as f.m.p.
				\item Para famílias de distribuições contínuas, $\mathcal{\mu}$ é a medida de Lebesgue e $p_\theta$ são as f.d.p.
			\end{halfwideitemize}
			\item Uma estatística é uma transformação (mensurável) da amostra, i.e. uma função $T(\boldsymbol{X})$.
			\item No que segue, vamos supor que, para qualquer estatística, as distribuições condicionais de $\boldsymbol{X}|T(\boldsymbol{X})$ sob cada $P_\theta$ admitem uma  {\color{blue}densidade condicional (regular)} $p_{\boldsymbol{X}|T(\boldsymbol{X}),\theta}(\cdot|\cdot)$.
			\begin{halfwideitemize}
	...
	\begin{frame}{Estatística suficiente}
		\begin{halfwideitemize}
			...
			\item Sob nossas condições técnicas, a definição é equivalente a que a densidade condicional $p_{\boldsymbol{X}|\boldsymbol{T}(X),\theta}$ não dependa de $\theta$.
		\end{halfwideitemize}
	...
	\begin{frame}{Exemplo}
		...
		\begin{equation*}
			\begin{aligned}         
				...
				= \frac{\theta^{x+y} (1-\theta)^{2-x-y}\mathbf{1}\{x+y = 2t\}}{\binom{2}{2t} \theta^{2t}(1-\theta)^{1-2t}}= \mathbf{1}\{x+y = 2t\} \, ,
			\end{aligned}
		\end{equation*}
	...
	\begin{frame}{Estatística suficiente minimal}
		\begin{halfwideitemize}
			...
			\end{halfwideitemize}
			\item Note que, se $T$ é estatística suficiente e $T = M\circ U$, então $U$ é estatística suficiente.
			\begin{halfwideitemize}
